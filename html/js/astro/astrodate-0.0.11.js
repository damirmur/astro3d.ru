...
((($ad) => {
    ...
    $ad.OrbRadix = (planets = [], aspects = [], orb = 0) => {
        const orbAsp = {};
        if (orb) {
            planets.forEach((pl) => {
                pl = Number(pl);
    ...
    $ad.OrbTpansit = (planets = [], aspects = [], orb = 0) => {
        const orbAsp = {};
        if (orb) {
            planets.forEach((pl) => {
                pl = Number(pl);
    ...
    $ad.aspectRadixArr = (planets, aspects, orbis = 0) => {
        const orb = $ad.OrbRadix(Array.from(planets, (el, i) => planets[i][0]), aspects, orbis);
        const aspPlsArr = [];
    ...
    $ad.objPltoArr = (obj, ind = 0) => {//obj=obj.out
        arr=[];
        for (const [key, value] of Object.entries(obj.planets)) {
            arr.push(([key,[value[ind],obj.speed[key][ind]]]))
        }
        return arr;
    }
    ...
    $ad.aspectsPeriod = (data) => {
        ...
        const orbs = $ad.OrbRadix(planets, aspects,data.orbs);//{}
