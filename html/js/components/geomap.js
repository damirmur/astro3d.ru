...
@media print {
    ...
    #shadow;
    #value = { latitude: 51.505, longitude: -0.09 };
    get value() {
        return this.#value;
    };
    set value(val) {
        ...
        this.#map.setView([this.#value.latitude, this.#value.longitude], 10);
    };
    static #source = { url: 'https://tile.openstreetmap.org/{z}/{x}/{y}.png', options: { minZoom: 2, maxZoom: 10, attribution: '&copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a>' } }
    get source() {
        return GeoMap.#source;
    };
    set source(val) {
        GeoMap.#source = val||GeoMap.#source;
        this.#render();
    };
    #map;
    #render() {
        const tiles = L.tileLayer(this.source.url, this.source.options).addTo(this.#map);
    };
    connectedCallback() {
        this.#shadow = this.attachShadow({ mode: 'closed' });
        this.#shadow.innerHTML = GeoMap.#templateCSS + GeoMap.#templateHTML;
        this.#map = L.map(this.#shadow.querySelector('#map')).setView([this.value.latitude, this.value.longitude], 10);
        this.#map.on('click', (e) => {
            const lng = ((e.latlng.lng % 360 + 36180) % 360 - 180);
